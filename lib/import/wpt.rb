    ...
    def log_results
      unless ENV['HOSTEDGRAPHITE_APIKEY'].nil? || !test_complete?
        ...
        unless wpt['data']['runs']['1']['firstView'].nil?
          ...
          metrics += key + ".wpt.first_view.score.cache #{first_view['score_cache']}\n"
          metrics += key + ".wpt.first_view.score.cdn #{first_view['score_cdn']}\n"
          metrics += key + ".wpt.first_view.score.gzip #{first_view['score_gzip']}\n"
          metrics += key + ".wpt.first_view.score.keep_alive #{first_view['score_keep-alive']}\n"
          metrics += key + ".wpt.first_view.score.combine #{first_view['score_combine']}\n"
          metrics += key + ".wpt.first_view.score.compress #{first_view['score_compress']}\n"
        ...
        unless wpt['data']['runs']['1']['repeatView'].nil?
          ...
          metrics += key + ".wpt.repeat_view.score.cache #{repeat_view['score_cache']}\n"
          metrics += key + ".wpt.repeat_view.score.cdn #{repeat_view['score_cdn']}\n"
          metrics += key + ".wpt.repeat_view.score.gzip #{repeat_view['score_gzip']}\n"
          metrics += key + ".wpt.repeat_view.score.keep_alive #{repeat_view['score_keep-alive']}\n"
          metrics += key + ".wpt.repeat_view.score.combine #{repeat_view['score_combine']}\n"
          metrics += key + ".wpt.repeat_view.score.compress #{repeat_view['score_compress']}\n"
